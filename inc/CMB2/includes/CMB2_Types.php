...
class CMB2_Types {
	...
	public function text_money() {
		return ( ! $this->field->get_param_callback_result( 'before_field' ) ? '$ ' : ' ' ) . $this->input( array(
	...
	public function textarea( $args = array() ) {
		...
		return sprintf( '<textarea%s>%s</textarea>%s', $this->concat_attrs( $a, array(
			'desc',
			'value'
		) ), $a['value'], $a['desc'] );
	}
	...
	public function textarea_code() {
		return sprintf( '<pre>%s', $this->textarea( array(
	...
	public function colorpicker() {
		...
		elseif ( ! preg_match( '/#|rgb/i', $meta_value ) ) {
		...
		return $this->input( array( 'class' => 'cmb2-colorpicker cmb2-text-small pt-pb-color', 'value' => $meta_value, 'data-alpha' => 'true' ) );
	}
	...
	public function is_valid_img_ext( $file ) {
		...
		$is_valid_types = (array) apply_filters( 'cmb2_valid_img_types', array(
			'jpg',
			'jpeg',
			'png',
			'gif',
